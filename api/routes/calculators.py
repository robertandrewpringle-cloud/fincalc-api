...
def amortize(
    ...
    term_months: int = Query(..., gt=0, le=1200, description="Loan term in months (max 1200)"),
):
...
def compound_interest(
    ...
    compounds_per_year: int = Query(12, gt=0, description="Compounding frequency per year"),
):
...
def npv(
    rate: float = Query(..., gt=-100, description="Discount rate, must be > -100 (e.g. 10.0 for 10%)"),
...
def depreciation(
    ...
    useful_life_years: int = Query(..., gt=0),
